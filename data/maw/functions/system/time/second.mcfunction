...
execute if score #maw Phase matches 3 if score #maw second matches 600 as @a[scores={CurrentRole=2,TeamWerewolf=2}] run tellraw @s [{"text":"       人狼の名前が書かれた本が配布されました","color": "red","underlined": true}]
execute if score #maw Phase matches 3 if score #maw second matches 600 as @a[scores={CurrentRole=2,TeamWerewolf=2}] run loot give @s loot maw:item/books/werewolf/known_werewolf
